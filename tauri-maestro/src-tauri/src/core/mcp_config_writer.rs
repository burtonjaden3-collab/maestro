...
/// servers (identified by the presence of `MAESTRO_SESSION_ID` in env vars).
fn merge_with_existing(
    ...
    let mut final_servers: HashMap<String, Value> = if mcp_path.exists() {
        ...
        // Get existing servers, filter out old Maestro-managed ones
        existing
            .get("mcpServers")
            ...
            .map(|obj| {
                obj.iter()
                    .filter(|(_, v)| {
                        ...
                        // (no MAESTRO_SESSION_ID in env vars)
                        v.get("env")
...
pub async fn write_session_mcp_config(
    ...
    // Add Maestro MCP server with session-specific env vars
        ...
        mcp_servers.insert(
            ...
            json!({
                ...
                "env": {
                    ...
                    "MAESTRO_PROJECT_HASH": project_hash,
...
mod tests {
    ...
    fn test_merge_preserves_user_servers() {
        ...
        // Write an existing config with a user server
        let existing = json!({
            "mcpServers": {
                "user-server": {
                    "type": "stdio",
        ...
        new_servers.insert(
            "new-maestro".to_string(),
            json!({
                "type": "stdio",
                "command": "/usr/bin/new-maestro",
                "env": {
                    "MAESTRO_SESSION_ID": "new-session"
                }
            }),
        );
        ...
        // Old Maestro server should be removed (had MAESTRO_SESSION_ID)
        assert!(!servers.contains_key("old-maestro"));
        // New Maestro server should be added
        assert!(servers.contains_key("new-maestro"));
    }
}
